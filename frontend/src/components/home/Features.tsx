...
const FeatureCard: React.FC<{ icon: React.ReactNode; title: string; description: string; href?: string; onClick?: (e: React.MouseEvent<HTMLAnchorElement>) => void }> = ({ icon, title, description, href, onClick }) => {
    const cardClasses = "bg-white p-8 rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 group h-full flex flex-col";
    ...
    if (href) {
        return <a href={href} className={cardClasses} onClick={onClick}>{content}</a>
    }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {features.map((feature) => (
            <FeatureCard
              key={feature.title}
              {...feature}
              onClick={feature.href ? (e) => handleNavClick(e, feature.href!) : undefined}
            />
          ))}
        </div>
      </div>
