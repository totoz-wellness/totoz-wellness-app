...
const getTrend = (t: string) => TREND_CONFIG[t as keyof typeof TREND_CONFIG] || TREND_CONFIG.INSUFFICIENT_DATA;

// ─── METRIC CARD ─────────────────────────────────────────────────────────────

const MetricCard: React.FC<{ label: string; value: string; accent: string; icon: React.ReactNode }> = ({
  label, value, accent, icon,
}) => (
  <div className="bg-white rounded-2xl border border-[#1e3a6e]/8 shadow-sm p-5">
    <div className="w-9 h-9 rounded-xl flex items-center justify-center mb-3" style={{ backgroundColor: accent + '18', color: accent }}>
      {icon}
    </div>
...
const BarRow: React.FC<{ label: string; count: number; max: number; rank: number; accent: string }> = ({
...
const Insights: React.FC = () => {
  ...
  const selectClass = 'w-full px-4 py-2.5 bg-white border border-[#1e3a6e]/15 rounded-xl text-[#1e3a6e] text-sm focus:outline-none focus:border-[#e9924b]/50 transition-all';
  ...
  const handleExport = () => {
    ...
    const text = [
      ...
      `Period: ${period.toUpperCase()} | Tracking: ${data.trackedPerson.type === 'SELF' ? 'Myself' : data.trackedPerson.name}`,
      '',
      '--- SUMMARY ---',
    ...
    const a = Object.assign(document.createElement('a'), { href: url, download: `growtrack-insights-${period}-${Date.now()}.txt` });
        ...
        <div className="bg-[#1e3a6e] py-12 relative overflow-hidden">
          ...
          <div className="relative z-10 max-w-6xl mx-auto px-6 md:px-12 lg:px-20">
            ...
            <div className="flex items-center justify-between flex-wrap gap-4">
              ...
              {data && (
                <button onClick={handleExport} className="hidden md:flex items-center gap-2 px-4 py-2.5 bg-white/10 border border-white/20 text-white text-sm font-semibold rounded-xl hover:bg-white/20 transition-all">
        ...
        <div className="max-w-6xl mx-auto px-6 md:px-12 lg:px-20 py-10">
          ...
          {loading ? (
            ...
            <div className="bg-white rounded-2xl border border-[#1e3a6e]/8 shadow-sm p-14 text-center">
              ...
              <p className="text-[#1e3a6e]/45 text-sm mb-8">Create entries to start generating insights.</p>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <MetricCard label="Total entries" value={data.totalEntries.toString()} accent="#659ec3" icon={<Calendar className="w-4 h-4" />} />
                <MetricCard label="Avg mood" value={`${data.metrics.averageMoodIntensity}/10`} accent="#e9924b" icon={<TrendingUp className="w-4 h-4" />} />
                <MetricCard label="Most common" value={data.metrics.predominantMood} accent="#1e3a6e" icon={<Brain className="w-4 h-4" />} />
                <MetricCard label="Mood variety" value={`${data.metrics.moodVariety} moods`} accent="#659ec3" icon={<Lightbulb className="w-4 h-4" />} />
              </div>
                  ...
                  <div className="bg-white rounded-2xl border border-[#1e3a6e]/8 shadow-sm p-6">
                    ...
                    <div className="space-y-4">
                      {data.metrics.topBehaviors.map((b, i) => (
                        <BarRow key={b.behavior} label={b.behavior} count={b.frequency} max={data.metrics.topBehaviors[0].frequency} rank={i + 1} accent="#659ec3" />
                      ))}
                    </div>
                  </div>
                )}
                  ...
                  <div className="bg-white rounded-2xl border border-[#1e3a6e]/8 shadow-sm p-6">
                    ...
                    <div className="space-y-4">
                      {data.metrics.topTriggers.map((t, i) => (
                        <BarRow key={t.trigger} label={t.trigger} count={t.frequency} max={data.metrics.topTriggers[0].frequency} rank={i + 1} accent="#e9924b" />
                      ))}
                    </div>
                  </div>
                )}
              ...
              <div className="bg-[#1e3a6e] rounded-2xl p-8">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    ...
                    <p className="text-white/45 text-xs mt-0.5">
                      For {data.trackedPerson.type === 'SELF' ? 'you' : data.trackedPerson.name}
                    </p>
                  </div>
                </div>
                ...
                <p className="text-white/25 text-xs mt-5 leading-relaxed">
                  These insights are generated by AI from your tracking data. They are not a substitute for professional mental health advice.
                </p>
              </div>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-white rounded-2xl border border-[#1e3a6e]/8 shadow-sm px-6 py-4">
                <p className="text-[#1e3a6e]/35 text-xs">
                  Generated {new Date(data.generatedAt).toLocaleString()} · {data.dateRange.start} to {data.dateRange.end}
                </p>
                <button onClick={handleExport} className="flex items-center gap-2 px-4 py-2 bg-[#1e3a6e] text-white text-xs font-semibold rounded-xl hover:bg-[#1e3a6e]/90 transition-all md:hidden">
